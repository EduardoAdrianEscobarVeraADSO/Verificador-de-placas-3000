...
async function consultarPlacas(event) {
  ...
  botonConsultar.disabled = !0;
  botonConsultar.innerText = "Consultando...";
  ...
  botonConsultar.disabled = !1;
  botonConsultar.innerText = "Consultar";
  alert(result.message);
  document.getElementById("download-btn").style.display = "block";
  document.getElementById("download-btnWord").style.display = "block";
}
async function descargarCartas() {
  const response = await fetch("/descargar-cartas", { method: "GET" });
  if (response.ok) {
    const blob = await response.blob();
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "cartas.zip";
...
async function enviarCorreo() {
  const response = await fetch("/enviar-correos", { method: "POST" });
  if (response.ok) {
    const blob = await response.blob();
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement("a");
    ...
    alert("Error al enviar correos: " + response.statusText);
  }
}
async function cargarTabla() {
  try {
    ...
    if (datosFiltrados.length > 0) {
          ...
          item.tabla_multa.forEach((multa) => {
            if (
              ...
              fila.innerHTML = `
                                ...
                                <td>${multa.estado}</td>
                                <td>${
                                  multa.valor_a_pagar.split("Detalle Pago")[0]
                                }</td>
                            `;
      ...
      tablaContenedor.innerHTML =
        "<p>No hay comparendos o multas pendientes.</p>";
    }
  } catch (error) {
