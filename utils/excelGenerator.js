...
function processJsonData(jsonData) {
  ...
  jsonData.forEach((item) => {
    ...
    const baseRow = {
      ...
      Documento_propietario: item.ID || "N/A",
    ...
    if (item.tabla_multa && item.tabla_multa.length > 0) {
      item.tabla_multa.forEach((mult) => {
        ...
        let infraccion = infra.match(
          /^(A0[1-9]|A1[0-2]|B0[1-9]|B1[0-9]|B2[0-3]|C0[1-9]|C1[0-9]|C2[0-9]|C3[0-9]|C[0-9][0-9]|D0[1-9]|D1[0-5]|E0[1-4]|F(0[1-9]|1[0-2])?|F\.0\.[23]|F\.[1-3]\.[1-3]|G0[1-2]|H0[1-9]|H1[0-3]|I0[1-2]|J0[1-6]|500|944)/
        );
        ...
        let resultadoFinal = resultado
          ? "$ " + resultado[0]
          : "Valor no disponible";
        let infraccionFinal = infraccion
          ? infraccion[0]
          : "Infracción no disponible";
...
function generateExcelForUsersAndPlates(data, fileName) {
  ...
  const usuariosActivos = data.usuariosActivos.map((usuario) => [usuario]);
  const placasActivas = data.placasActivas.map((placa) => [placa]);
  const usuariosSheet = XLSX.utils.aoa_to_sheet([
    ["Usuarios Activos"],
    ...usuariosActivos,
  ]);
  XLSX.utils.book_append_sheet(workbook, usuariosSheet, "Usuarios Activos");
  const placasSheet = XLSX.utils.aoa_to_sheet([
    ["Placas Activas"],
    ...placasActivas,
  ]);
